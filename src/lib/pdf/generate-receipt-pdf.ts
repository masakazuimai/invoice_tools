...
export async function generateReceiptPdf(
  ...
  // --- タイトル ---
  doc.font("Bold").fontSize(22).text("領収書", leftX, y, { align: "center" })
  y += 40

  // --- 宛先 ---
  doc.font("Regular")
  if (receipt.customer.zipCode) {
    doc.fontSize(9).text(`〒${receipt.customer.zipCode}`, leftX, y)
    y += 14
  }
  if (receipt.customer.address) {
    doc.fontSize(9).text(receipt.customer.address, leftX, y)
    y += 14
  }
  const customerY = y
  doc.font("Bold").fontSize(14).text(`${receipt.customer.name} 様`, leftX, y)
  y += 30
  ...
  doc.font("Regular").fontSize(10).text("下記の金額を領収いたしました。", leftX, y)
  y += 25

  doc.font("Bold").fontSize(24).text(formatCurrency(receipt.totalAmount), leftX, y, { align: "center" })
  y += 40
  ...
  doc.text(`但し書き: ${receipt.subject ?? "-"}`, leftX, y)
  y += 16
  ...
  if (receipt.totalAmount >= 50000) {
    ...
    doc.fontSize(7).fillColor("#999999").text("収入印紙", stampX, stampY + 20, { width: 80, align: "center" })
  }
  ...
  if (receipt.notes) {
    ...
    y += 12
    doc.font("Regular").fontSize(9).fillColor("#000000").text(receipt.notes, leftX, y, { width: CONTENT_WIDTH })
  }
