...
import type { QuotationItemInput } from "@/schemas/quotation.schema"
...
export function QuotationForm({ initialData }: Props) {
  ...
  // 有効期限のデフォルト: 発行日から30日後
  useEffect(() => {
    if (!validUntil && issueDate) {
      const d = new Date(issueDate)
      d.setDate(d.getDate() + 30)
      setValidUntil(toDateString(d))
    }
  }, [issueDate, validUntil])

  const updateItem = useCallback((index: number, field: keyof QuotationItemInput, value: string | number) => {
    setItems((prev) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { customerId, issueDate, validUntil, subject, items, notes }
